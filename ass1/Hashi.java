/*
 * Algorithm:
 * The solver employs a recursive backtracking algorithm to find a solution to the puzzle.
 * It starts by finding the first island on the puzzle grid.
 * Then, it attempts to connect bridges from this island to other islands, recursively exploring possible configurations until a solution is found or all possibilities are exhausted.
 * If a bridge connection is made successfully, the solver continues the recursive search from the newly connected island.
 * If a solution is found, it returns the solved puzzle grid; otherwise, it returns null.
 * 
 * Data Structures:
 * The puzzle grid is represented as a 2D array of Square objects, which allows for easy manipulation and traversal of the grid.
 * Each Square object keeps track of its position and connections to other squares.
 * The Island class represents islands on the grid, keeping track of their number and the number of bridges connected in each direction (up, down, left, right).
 * The Empty class represents empty spaces on the grid, keeping track of the number of bridges allowed to be connected in each direction.
 * The Direction enum represents the four possible directions in which bridges can be built.
 * 
 * Design Decisions:
 * The program employs object-oriented principles to model the puzzle grid and its components (islands, empty spaces, bridges) using classes and inheritance.
 * Recursion is used to explore possible bridge connections from each island, making the solver concise and easy to understand.
 * The program uses sets to efficiently handle duplicate entries and ensure that each island is visited only once during the solution search.
 * Copying the board at each step of the recursion allows the solver to explore different configurations without modifying the original puzzle grid.
 * Error handling is included to ensure that the input puzzle grid is correctly parsed and that the solver gracefully handles invalid configurations.
 */
